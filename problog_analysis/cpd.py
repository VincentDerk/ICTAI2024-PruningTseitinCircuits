    ...
    def to_problog(
        ...
        for k, v in table:
            ...
            if var.boolean_true is None:
                _convert_one_to_None = lambda prob_val: None if drop_zero and prob_val == 1.0 else prob_val
                head_problits = [(_convert_one_to_None(vv), var.to_problog_value(var.values[idx], value_as_term)) for idx, vv in enumerate(v) if not (drop_zero and vv == 0.0)]
            ...
            if ad_is_function:
                ...
                elif len(head_problits) == 1 and head_problits[0][1].startswith("\\+"):
                    # this is a negative fact \+some_fact.
                    # problog needs a clause with some_fact in the head.
                    # so we change it to some_fact :- false.
                    prob, fact_str = head_problits[0]
                    head_problits = [(prob, fact_str.replace("\\+", ""))]
                    body_str = " :- false"
                else:
                    body_str = ""
                _format_headlit = lambda _prob, _lit: f"{_prob}::{_lit}" if _prob is not None else str(_lit)
                head_strs = [_format_headlit(prob, lit) for prob, lit in head_problits]
